...
def main():
    ...
    # Lightweight companion file with everything except the full review
    # text/author list - this is what the player-count dashboard fetches
    # to overlay review activity on top of online-count charts, so it
    # doesn't need to pull down the (much larger) full review text just
    # to read a handful of summary numbers.
    summary_only_path = os.path.join(os.path.dirname(args.latest_out), "summary-only.json")
    summary_only = {
        "appid": args.appid,
        "appname": args.appname,
        "generated_at": now_iso,
    ...
    with open(summary_only_path, "w", encoding="utf-8") as f:
        json.dump(summary_only, f, ensure_ascii=False, separators=(",", ":"))
    print(f"Wrote {summary_only_path}")
